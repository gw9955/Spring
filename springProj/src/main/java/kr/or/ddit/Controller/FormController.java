...
//골뱅이ModelAttribute 애너테이션으로 폼 객체의 속성명을 직접 지정할 수 있음
	...
	public String registerForm03(@ModelAttribute("memVO") MemVO memVO, Model model) {
		log.info("registerForm03");

		// 폼 객체의 프로퍼티에 값을 미리지정해놓음
		memVO.setMemId("gaddongi");
	...
	// 요청URI : /form/register
	// 방식 : post
	// 파라미터 : {"memId" : "gaeddongi","memName":"개똥이", "password
	// ":"java","introdcution":"소개글",
	// "hobbyList":"[01,03]"}
	@PostMapping("/register")
	public String registerPost(@Validated MemVO memVO, Model model, BindingResult result) {
		log.info("memVO : " + memVO.toString());

		// registerForm03.jsp에서 post 요청시 Validated 확인 후
		// 문제 발생 시 폼 화면으로 돌아감
		if (result.hasErrors()) {
			return "form/registerForm03";
		}
